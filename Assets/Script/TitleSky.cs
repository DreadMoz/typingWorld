...
//        gm.savedata.testEncodeMedals();   // Medalデバッグ
    ...
    public void finishOAuth(string userInfo)
    {
        ...
        if ((mailText.text.Substring(mailText.text.Length - 13) == "e-net.nara.jp") || gm.gmailToggle.isOn)
        {
            ...
            gm.connection.loadExtension();      // あしあとデータサーチ
        }
        else
        {
    ...
    public void testFinishDataLoad()
    {
        ...
        finishDataLoadExt(dummyJson);
    }

    public void finishDataLoadExt(string msg)
    {
        message.SetActive(true);
        Text messageText = message.GetComponentInChildren<Text>();
        ...
        {
            messageText.text = "あしあとデータを読み込む時にエラーが発生しました: " + ex.Message;
    ...
    private void checkExtensionData()
    {
        ...
        if (gm.savedata.Equipment[eq.CatBody] == 0)        // ねこボディなし
        {
            if (gm.savedata.Settings[se.Extension] == 0)        // 拡張機能なしの判定なら
            {
                ...
                messageText.text = "あしあとデータがないので、クラウドから取ってきます。";
            }
            else        // 拡張機能ありの判定なら
            {
                Debug.Log("拡張機能ありねこボディなし、GASアクセスへ");
                messageText.text = "あしあとデータが空なので、クラウドから取ってきます。";
            }
    ...
    public void confirmNeco()
    {
        ...
        if (gm.savedata.Settings[se.Extension] == 1)
        {
            gm.exportExtension();  // 拡張機能に保存
        }
